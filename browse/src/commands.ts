...
export const COMMAND_DESCRIPTIONS: Record<string, { category: string; description: string; usage?: string }> = {
  ...
  'fetch':    { category: 'Web', description: 'URL → clean markdown via Firecrawl (PDF-aware), auto-falling back to the local browser on failure/empty/no-key. --full returns the whole page (not just main content); --wait <ms> for slow JS renders; --engine forces firecrawl|browser.', usage: 'fetch <url> [--html] [--links] [--full] [--wait <ms>] [--engine firecrawl|browser]' },
  // Visual
