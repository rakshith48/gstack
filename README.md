...
| `/web-search` | **Researcher** | Search the web via Firecrawl — real results, each scraped to clean markdown inline so you answer with citations, not links. Pairs with `$B fetch <url>` for faithful URL→markdown. Needs a Firecrawl key (`FIRECRAWL_API_KEY`, `gstack-config set firecrawl_key`, or `npx firecrawl-cli login`); without one, fetch falls back to the local browser and search hands off to native web search. |
